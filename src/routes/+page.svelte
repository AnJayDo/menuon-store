<script lang="ts">
	...
	let menuList: ITag[] = data.tags.filter((item: ITag) =>
		item.categories.includes(activeCatagoriesId)
	);
	...
	function handleChangeCatagory(id: string) {
		...
		if (chosenMenu) {
			updateMenuListByCat(chosenMenu?.id);
			updateFoodListByCat(chosenMenu?.id);
		}
	...
	function updateMenuListByCat(menuCatId: string) {
		menuList = data.tags.filter((item: ITag) => item.categories.includes(menuCatId));
	}
